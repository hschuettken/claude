...
async def init_pool() -> None:
    ...
    try:
        ...
        logger.error("agent_economy db_pool_failed error=%s", exc)
        _pool = None
...
async def _run_migrations() -> None:
    ...
    for sql_file in sorted(migrations_dir.glob("*.sql")):
        try:
            ...
            logger.info("agent_economy migration_applied file=%s", sql_file.name)
        except Exception as exc:
            logger.warning("agent_economy migration_skipped file=%s error=%s", sql_file.name, exc)
