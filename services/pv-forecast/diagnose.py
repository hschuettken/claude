...
Usage:
    ...
    docker compose run --rm pv-forecast python diagnose.py --step nats
...
def check_config() -> dict:
    ...
    try:
        ...
        checks = {
            ...
            "INFLUXDB_TOKEN": s.influxdb_token[:8] + "..."
            if s.influxdb_token
            else "(empty)",
            ...
            "FORECAST_SOLAR_EAST_ENTITY_ID": s.forecast_solar_east_entity_id
            or "(not set)",
            "FORECAST_SOLAR_WEST_ENTITY_ID": s.forecast_solar_west_entity_id
            or "(not set)",
        }
...
def check_influx(settings) -> None:
    ...
    influx = InfluxClient(
        settings.influxdb_url, settings.influxdb_token, settings.influxdb_org
    )
    try:
        # Test basic connectivity with a simple query
        tables = influx.query_raw(
            f'buckets() |> filter(fn: (r) => r.name == "{settings.influxdb_bucket}")'
        )
        found = any(settings.influxdb_bucket in str(t) for t in tables)
        result(
            "Connection", True, f"Bucket '{settings.influxdb_bucket}' found: {found}"
        )
        ...
        for label, entity_id in [
            ...
            try:
                ...
                if records:
                    ...
                    warn(
                        f"{label} ({entity_id}): 0 records in last 7 days",
    ...
    try:
        ...
        if records:
            daytime = [
                r
                for r in records
                if r.get("shortwave_radiation", 0)
                and r.get("shortwave_radiation", 0) > 0
            ]
            # Show sunrise/sunset info
            sr = records[0].get("sunrise_hour", "?")
            ss = records[0].get("sunset_hour", "?")
            sr_str = (
                f"{int(sr)}:{int((sr % 1) * 60):02d}"
                if isinstance(sr, float)
                else str(sr)
            )
            ss_str = (
                f"{int(ss)}:{int((ss % 1) * 60):02d}"
    ...
    try:
        ...
        for array, res in train_results.items():
            ...
            if status == "trained":
                ...
                info(
                    f"Train {array}: using fallback (only {res.get('days', 0)} days of data, need {settings.model_min_days})"
                )
            else:
        ...
        for arr_name, arr in [("East", forecast.east), ("West", forecast.west)]:
            if arr and arr.today and arr.today.hourly:
                hours = [
                    f"{h.time.strftime('%H:%M')}={h.kwh}" for h in arr.today.hourly
                ]
...
async def main() -> None:
    ...
    parser.add_argument(
        "--step",
        choices=["config", "ha", "influx", "nats", "weather", "forecast", "all"],
        default="all",
