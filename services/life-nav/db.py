...
async def init_pool() -> None:
    ...
    if not settings.db_url:
        logger.info("LIFE_NAV_DB_URL not set — running without persistent storage")
        return
    try:
        _pool = await asyncpg.create_pool(settings.db_url, min_size=2, max_size=10)
        logger.info("life_nav db_pool_created dsn=%s", settings.db_url.split("@")[-1])
        await _run_migrations()
    except Exception as exc:
        logger.error("life_nav db_pool_failed error=%s", exc)
        _pool = None
...
async def _run_migrations() -> None:
    ...
    for sql_file in sorted(migrations_dir.glob("*.sql")):
        try:
            ...
            logger.info("life_nav migration_applied file=%s", sql_file.name)
        except Exception as exc:
            logger.warning("life_nav migration_skipped file=%s error=%s", sql_file.name, exc)
